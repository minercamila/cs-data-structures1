...
def sum_zero_1(numbers):
    ...
    for x in s:                         # for loop is linear 
        if -x in s:                     # checking set is constant                    
            result.append([-x, x])      # append = constant
...
def sum_zero_2(numbers):
    ...
    for x in numbers:                   # nested for loop = exponential
        for y in numbers:
            if x == -y:
                result.append((x, y))
    return result
